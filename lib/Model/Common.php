...
class Common implements ModelInterface, ArrayAccess
{
    ...
    const OPERATION_INITIATOR_TYPE__0 = '0';
    const OPERATION_INITIATOR_TYPE__1 = '1';
    const OPERATION_INITIATOR_TYPE__2 = '2';
    const OPERATION_INITIATOR_TYPE_R = 'R';
    const OPERATION_INITIATOR_TYPE_I = 'I';
    ...
    public function __construct(array $data = null)
    {
        ...
        $this->container['operation_initiator_type'] = isset($data['operation_initiator_type']) ? $data['operation_initiator_type'] : null;
    }
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['operation_initiator_type']) && !in_array($this->container['operation_initiator_type'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value for 'operation_initiator_type', must be one of '%s'",
     ...
     *
     * @param string|null $operation_initiator_type Признак инициатора операции * '0' - Оплата без сохранения реквизитов карты для последующего использования. Cценарий \"0 - CIT. Credential-Not-Captured\" * '1' - Используется, если Мерчант сохраняет карту. Cценарий - \"1 - Consumer-Initiated, Credential-Captured\" * '2' - Операция по ранее сохранённой карте, инициирована клиентом. Cценарий - \"2 - Consumer-Initiated, Credential-on-File\" * 'R' - Повторяющаяся операция по сохранённой карте без графика. Является Merchant Initiated сценарием (\"R = Merchant-Initiated, Credential-on-File, Recurring\") * 'I' - Повторяющаяся операция по сохраненной карте  в соответствии с графиком платежей для погашения кредита. Является Merchant Initiated сценарием (\"I = Merchant-Initiated, Credential-on-File, Installment\")
     *
     * @return $this
     */
