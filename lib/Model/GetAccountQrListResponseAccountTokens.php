...
/**
 * GetAccountQrListResponseAccountTokens Class Doc Comment
 *
 * @category Class
 ...
 * @link     https://openapi-generator.tech
 */
class GetAccountQrListResponseAccountTokens implements ModelInterface, ArrayAccess
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'GetAccountQrListResponse_AccountTokens';
     ...
     * Sets status
     *
     * @param string|null $status Статус привязки карты: * NEW - получен запрос на привязку счета; * PROCESSING - запрос в обработке; * ACTIVE - привязка счета успешна; * INACTIVE - привязка счета неуспешна/деактивирована
     *
     * @return $this
     */
    public function setStatus($status)
    {
        ...
        if (!is_null($status) && (mb_strlen($status) > 20)) {
            throw new \InvalidArgumentException('invalid length for $status when calling GetAccountQrListResponseAccountTokens., must be smaller than or equal to 20.');
        }
     ...
     * Sets bank_member_id
     *
     * @param string|null $bank_member_id Идентификатор банка клиента (эмитент), который будет совершать оплату по привязаному счету - заполнен, если статус ACTIVE, INACTIVE
     *
     * @return $this
     */
