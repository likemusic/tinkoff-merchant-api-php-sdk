...
/**
 * QrResponseSDK Class Doc Comment
 *
 * @category Class
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech
 */
class QrResponseSDK implements ModelInterface, ArrayAccess
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'QrResponse_SDK';
    ...
    public function setTerminalKey($terminal_key)
    {
        if ((mb_strlen($terminal_key) > 20)) {
            throw new \InvalidArgumentException('invalid length for $terminal_key when calling QrResponseSDK., must be smaller than or equal to 20.');
        }
     ...
     * Sets data
     *
     * @param string $data В зависимости от параметра DataType в запросе это:   * Payload - информация, которая должна быть закодирована в QR   * SVG изображение QR в котором уже закодирован Payload
     *
     * @return $this
     */
    ...
    public function setErrorCode($error_code)
    {
        if ((mb_strlen($error_code) > 20)) {
            throw new \InvalidArgumentException('invalid length for $error_code when calling QrResponseSDK., must be smaller than or equal to 20.');
        }
    ...
    public function setMessage($message)
    {
        if (!is_null($message) && (mb_strlen($message) > 255)) {
            throw new \InvalidArgumentException('invalid length for $message when calling QrResponseSDK., must be smaller than or equal to 255.');
        }
     ...
     * Sets request_key
     *
     * @param string $request_key Идентификатор запроса на привязку счета. Передается в случае привязки и одновременной оплаты по CБП.
     *
     * @return $this
     */
