...
class InlineObject2 implements ModelInterface, ArrayAccess
{
     ...
     * Sets md
     *
     * @param string $md Уникальный идентификатор транзакции в системе Тинькофф Кассы (возвращается в ответе от ACS)
     *
     * @return $this
     */
     ...
     * Sets payment_id
     *
     * @param string|null $payment_id Уникальный идентификатор транзакции в системе Тинькофф Кассы
     *
     * @return $this
     */
     ...
     * Sets terminal_key
     *
     * @param string|null $terminal_key Идентификатор терминала, выдается Мерчанту Тинькофф Кассой
     *
     * @return $this
     */
    public function setTerminalKey($terminal_key)
    {
        if (!is_null($terminal_key) && (mb_strlen($terminal_key) > 20)) {
            throw new \InvalidArgumentException('invalid length for $terminal_key when calling InlineObject2., must be smaller than or equal to 20.');
        }
     ...
     * Sets token
     *
     * @param string|null $token Подпись запроса
     *
     * @return $this
     */
