...
class Cancel implements ModelInterface, ArrayAccess
{
    ...
    public function __construct(array $data = null)
    {
        ...
        $this->container['qr_member_id'] = isset($data['qr_member_id']) ? $data['qr_member_id'] : null;
        $this->container['route'] = isset($data['route']) ? $data['route'] : null;
        $this->container['source'] = isset($data['source']) ? $data['source'] : null;
        $this->container['external_request_id'] = isset($data['external_request_id']) ? $data['external_request_id'] : null;
    }
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['route']) && !in_array($this->container['route'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value for 'route', must be one of '%s'",
     ...
     * Sets terminal_key
     *
     * @param string $terminal_key Идентификатор терминала выдается Мерчанту Тинькофф Кассой
     *
     * @return $this
     */
     ...
     * Sets payment_id
     *
     * @param float $payment_id Идентификатор платежа в системе Тинькофф Кассы
     *
     * @return $this
     */
     ...
     * Sets amount
     *
     * @param float|null $amount Сумма в копейках (если не передан, используется `Amount`, переданный в методе **Init**). В случае отмены статуса **NEW** поле `Amount`, даже если оно заполнено, игнорируется. Отмена производится на полную сумму
     *
     * @return $this
     */
     ...
     * Sets receipt
     *
     * @param object|null $receipt JSON-объект с данными чека. Обязателен, если подключена онлайн-касса.<br> Данные переданные в данном запросе могут отличаться от переданных в **Init**, если отмена производится только по части товаров. <br> В случае полной отмены структура чека не передается. В случае частичной отмены необходимо передавать те товары, которые нужно отменить.
     *
     * @return $this
     */
     ...
     * Sets shops
     *
     * @param \OpenAPI\Client\Model\Shops[]|null $shops Обязательный для маркетплейсов. JSON обьект с данными Маркетплейса.
     *
     * @return $this
     */
     ...
     * Sets qr_member_id
     *
     * @param string|null $qr_member_id Код банка в классификации СБП, в который необходимо выполнить возврат. См. параметр `MemberId` в методе [**QrMembersList**](#tag/Oplata-cherez-SBP/paths/~1QrMembersList/post).
     *
     * @return $this
     */
     ...
     * Sets route
     *
     * @param string|null $route Способ платежа.  * При проведении платежа в «Рассрочку» необходимо передавать значение TCB * При проведении платежа «Долями» необходимо передавать значение BNPL
     *
     * @return $this
     */
     ...
     * Sets source
     *
     * @param string|null $source Источник платежа.  * При проведении платежа в «Рассрочку» необходимо передавать значение installment * При проведении платежа «Долями» необходимо передавать значение BNPL
     *
     * @return $this
     */
